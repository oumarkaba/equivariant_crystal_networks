...
def draw_colored_bipartite_graph(n, m, colors, edge_width=2.0):
    ...
    nx.draw_networkx_edges(
        G, pos=pos, width=edge_width, edge_color=c, edge_cmap=plt.cm.viridis, alpha=0.6
    )
...
def draw_colored_matrix(n, m, colors):
    if isinstance(colors, dict):
        ...
        for i in range(n):
            for j in range(m):
                x[n-i-1, j] = colors.get((i, j), -1)
    else:
        x = colors
    plt.pcolormesh(x, cmap="jet", edgecolors='k', linewidth=2)
    ax = plt.gca()
    ax.set_aspect('equal')
    fig = ax.figure
    plt.axis("off")
